...
describe('Validation TEST', function() {
  ...
  describe('Test authorization header',async function(done) {
    it('should throw 401 if no authorization header', async function(done) {
        var app = new koa();
    ...
    it('should throw 404 if authorization header is present', async function(done){
        var app = new koa();
        app.use(koajwt({ secret: 'shhhh' }));
        request(app.listen())
          .get('/')
    ...
    it('should throw 401 with bad formated token', async function(done) {
        var app = new koa();
        app.use(koajwt({ secret: 'shhhh' }));
        request(app.listen())
          .get('/')
    ...
    it('should throw 401 with invalid token JWT',async function(done) {
        var app = new koa();
        app.use(koajwt({ secret: 'shhhh' }));
        request(app.listen())
          .get('/')
    ...
    it('should throw 404 with valid token',async function(done) {
      var app = new koa();
      app.use(koajwt({ secret: 'shhhh' }));
      request(app.listen())
        .get('/')
    ...
    it('should throw 401 with expired token',async function(done) {
      var app = new koa();
      app.use(koajwt({ secret: 'shhhh' }));
      request(app.listen())
        .get('/')
